...
public class PasswordUtilTests {
    ...
    public void isNotComplexPassword() {
        ...
        Assert.assertFalse("It is a complex password: " + password,
                PasswordUtil.isComplexPassword(password, password.length()));
    }

    @Test
    public void allComplexPasswords() {
        for (String password : new String[]{"!abC1efgh", "abC1e@g)", "abC1e_gh8",}) {

            Assert.assertTrue("String is not complex enough: " + password,
                    PasswordUtil.isComplexPassword(password, password.length()));
    ...
    public void allNotComplexPasswords() {
        for (String password : new String[]{"abcdefg", "abcdefgh", "abCdefgh", "abc1efgh", "1_g"}) {

            Assert.assertFalse("String is complex enough: " + password,
                    PasswordUtil.isComplexPassword(password, password.length()));
